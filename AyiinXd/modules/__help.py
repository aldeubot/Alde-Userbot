    ...
    if args:
        ...
        if BOT_USERNAME:
            ...
            try:
                ...
                # Jika Inline Mode tidak aktif, aktifkan inline mode
                ...
                async with bot.conversation(chat) as conv:
                    try:
                        ...
                        # Unblock @BotFather jika terblokir
                    ...
                    # Beritahu pengguna bahwa inline mode sudah diaktifkan
                    await xx.edit(f"**Berhasil Menyalakan Mode Inline**\n\n**Ketik** `{cmd}help` **lagi untuk membuka menu bantuan.**")
                
                # Hapus pesan yang digunakan untuk mengaktifkan inline mode
                await bot.delete_messages(
                    conv.chat_id,
                    [first.id, second.id, third.id, fourth.id, fifth.id, sixth.id],
                )
        else:
            await eor(event, "**Silahkan Buat BOT di @BotFather dan Tambahkan Var** `BOT_TOKEN` & `BOT_USERNAME`")
